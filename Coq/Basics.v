...
(** Using pattern matching, it is not too difficult to define the
    ...
    recalls that we can match against _two_ values simultaneously by
...
(** As a further sanity check, we can prove that the [letter_comparison] function
    does indeed give the result [Eq] when comparing a letter [l] against itself.
*)
...
(** We can follow the same strategy to define the comparison operation for 
    two grade modifiers.  We consider them to be ordered as [Plus > Natural > Minus ].
*)
...
(** **** Exercise: 2 stars, standard (grade_comparison)
    ...
    the letters are equal do we need to consider the
    modifiers. (i.e. all grade variants of [A] are greater than all
    grade variants of [B]).
...
(** The problem, of course, has to do with the "edge case" of lowering
    [F], which we can identify using this easy-to-prove fact: *)
...
(** Now we can state a better version of the lower letter theorem that
    ...
    says that [F] is stictly smaller than [l], which rules out the
...
(** **** Exercise: 3 stars, standard (lower_grade_lowers)
    ...
    _not_ a good idea.  Instead, more judicious use of [destruct],
    along with rewriting is a better strategy.
    ...
    destruct a [letter].  The case for [F] will probably benefit
    from [lower_grade_F_Minus].
*)
...
(** Now we can prove some properties about this policy. *)
...
(** With the tools that we have available so far, we cannot yet
    (easily) prove more interesting properties about this late policy.
*)
...
Fixpoint bin_to_nat (m:bin) : nat :=
  ...
  | B1 b => plus (mult 2 (bin_to_nat b)) 1
  end.
